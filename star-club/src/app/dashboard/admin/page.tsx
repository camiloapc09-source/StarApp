    ...
    <div>
      ...
      <div className="p-8 space-y-8">
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          ...
          <StatCard label={t.common.overduePayments} value={overduePayments} icon={<TrendingUp size={20} />} iconColor="text-error" />
        </div>
          ...
          <div className="lg:col-span-2">
            <Card>
              <div className="flex items-center justify-between mb-6">
                ...
                <Link href="/dashboard/admin/players" className="text-sm text-accent hover:text-accent-dim transition-colors">
                  {t.common.viewAll}
                </Link>
              </div>
              <div className="space-y-3">
                {recentPlayers.length === 0 ? (
                  <p className="text-center py-6" style={{ color: "var(--text-muted)" }}>{t.common.noPlayersYet}</p>
                ) : (
                  recentPlayers.map((player) => (
                    <div key={player.id} className="flex items-center gap-3 p-3 rounded-xl hover:bg-[var(--bg-hover)] transition-all">
            ...
            <Card>
              ...
              <div className="space-y-2">
                {[
                  ...
                  { label: t.common.createSession ?? "Nueva sesión", href: "/dashboard/admin/attendance/new" },
                  { label: t.common.recordPayment ?? "Registrar pago", href: "/dashboard/admin/payments/new" },
                  { label: t.common.viewReports ?? "Reportes", href: "/dashboard/admin/reports" },
                ].map((action) => (
                  <Link key={action.href} href={action.href} className="block px-4 py-2.5 rounded-xl text-sm text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-hover)] transition-all">
                    {action.label}
                  </Link>
                ))}
              </div>
