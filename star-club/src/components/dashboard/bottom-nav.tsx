...
const ROLE_TABS: Record<string, (notif: number) => TabItem[]> = {
  ...
  coach: (_notif) => [
    ...
    { href: "/dashboard/coach/players",  icon: Users,           label: "Jugadores", matchPrefix: "/dashboard/coach/players" },
    { href: "/dashboard/coach/sessions", icon: Calendar,        label: "Sesiones",  matchPrefix: "/dashboard/coach/sessions" },
    { href: "/dashboard/coach/missions", icon: Target,          label: "Misiones",  matchPrefix: "/dashboard/coach/missions" },
  ],
  player: (_notif) => [
...
function MoreSheet({
  items, open, onClose, pathname,
}: {
  items: MoreItem[]; open: boolean; onClose: () => void; pathname: string;
}) {
  return (
    <AnimatePresence>
      {open && (
        <>
          ...
          >
            ...
            <div className="flex items-center justify-between px-6 py-3">
              ...
              <button onClick={onClose} className="p-1.5 rounded-xl" style={{ color: "rgba(255,255,255,0.35)" }}>
                <X size={16} />
              </button>
            </div>
                  ...
                  <Link key={item.href} href={item.href} onClick={onClose}>
                    <div
                      ...
                      style={{
                        ...
                        border: `1px solid ${active ? item.color + "4D" : "rgba(255,255,255,0.07)"}`,
                      }}
                    >
